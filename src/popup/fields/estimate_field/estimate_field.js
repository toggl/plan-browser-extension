...
import template from './estimate_field.dot';
import './estimate_field.scss';
...
const EstimateField = View.extend({
  ...
  events: {
    ...
    'focus .estimate-field__input': 'focusInput',
    click: 'startEditing',
  ...
  bindings: {
    prompt: [
      { type: 'booleanClass', name: 'prompt' },
      { type: 'booleanClass', name: 'row--bg-light' },
      { type: 'booleanClass', name: 'spacing-2--inner-left' },
      { type: 'toggle', hook: 'prompt' },
    ],
    disabled: [
  ...
  initialize() {
    this.initInputVal();
    this.listenTo(this, 'change:value', () => this.initInputVal());
  },
  ...
  focusInput(e) {
    // this.el.querySelector('.estimate-field__input').select();
    e.target.select();
  },
